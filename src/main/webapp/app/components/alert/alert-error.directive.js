(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('jiraReportApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-jiraReportApp-error');
                var entityKey = httpResponse.headers('X-jiraReportApp-params');
                if (errorHeader) {
